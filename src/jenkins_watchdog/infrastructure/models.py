...
class JenkinsFailureReportRecord(Base):
    ...
    coverage_exceptions: Mapped[list[dict[str, Any]]] = mapped_column(JSON_TYPE, nullable=False, default=list)
...
class JenkinsFailureReportBuildRecord(Base):
    __tablename__ = "jenkins_failure_report_builds"
    __table_args__ = (
        UniqueConstraint("report_id", "build_id", name="uq_jenkins_failure_report_build"),
        Index("ix_jenkins_failure_report_builds_report_status", "report_id", "status", "id"),
    )

    id: Mapped[uuid.UUID] = mapped_column(Uuid(as_uuid=True), primary_key=True)
    report_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("jenkins_failure_reports.id", ondelete="CASCADE"), nullable=False
    )
    build_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("jenkins_builds.id", ondelete="CASCADE"), nullable=False)
    status: Mapped[str] = mapped_column(String(24), nullable=False, default="queued")
    investigation_request_id: Mapped[uuid.UUID | None] = mapped_column(
        ForeignKey("investigation_requests.id", ondelete="SET NULL")
    )
    error_summary: Mapped[str | None] = mapped_column(Text)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
